ARCH_FLAGS    ?= -mmacosx-version-min=10.13 -arch x86_64 -arch arm64
FRAMEWORK_PATH = -F/System/Library/PrivateFrameworks
FRAMEWORK      = -framework Carbon -framework Cocoa -framework CoreServices -framework SkyLight -framework ScriptingBridge
BUILD_FLAGS    = -std=c99 -Wall -g -O0 -fvisibility=hidden -fno-objc-arc $(ARCH_FLAGS)
BUILD_PATH     = ./bin
DOC_PATH       = ./doc
SCRIPT_PATH    = ./scripts
ASSET_PATH     = ./assets
...
install: BUILD_FLAGS=-std=c99 -Wall -DNDEBUG -O2 -fvisibility=hidden -fno-objc-arc $(ARCH_FLAGS)
install: clean-build $(BINS)

$(OSAX_SRC): $(OSAX_PATH)/loader.m $(OSAX_PATH)/payload.m
	xcrun clang $(OSAX_PATH)/loader.m -shared -O2 $(ARCH_FLAGS) -o $(OSAX_PATH)/loader -framework Foundation
	xcrun clang $(OSAX_PATH)/payload.m -shared -fPIC -O2 $(ARCH_FLAGS) -o $(OSAX_PATH)/payload -framework Foundation -framework Carbon
	xcrun clang $(OSAX_PATH)/mach_loader.m -O2 $(ARCH_FLAGS) -o $(OSAX_PATH)/mach_loader -framework Cocoa
